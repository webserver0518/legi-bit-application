/*************************************************
 * loader.admin.js – Independent Admin Loader
 * Public API: window.AdminLoader.load({ page, force })
 *             window.AdminLoader.navigate({ linkEl, page, force })
 **************************************************/
(function () {
    if (!window.Core?.storage) {
        throw new Error("AdminLoader: window.Core.storage must be loaded first");
    }
    ...
    async function loadInternal(page, force) {
        if (typeof force !== "boolean") {
            throw new Error("AdminLoader.load: 'force' must be boolean");
        }
